    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          'https://api-google-sheets-7zph.vercel.app/getDataEngResumo'
        );
        ...
        if (orçamentoLinha) {
          setOrçamento(Number(orçamentoLinha[1].replace('R$', '').replace(/\./g, '').replace(',', '.')));
        }
        if (gastoTotalLinha) {
          setGastoTotal(Number(gastoTotalLinha[1].replace('R$', '').replace(/\./g, '').replace(',', '.')));
        }
      } catch (error) {
